...
Going one level deeper means answering: where does truth live, where does behavior
live, and how does change propagate? Apply these heuristics (do not recite them):
- An aggregate is a consistency boundary, not a data grouping: one transactional
  invariant per aggregate, kept small, referencing other aggregates by ID only. If
  an aggregate keeps growing you are modeling a workflow — split it.
- A domain service holds behavior that belongs to no single aggregate
  (cross-aggregate logic, external calls, heavy computation); keep it stateless and
  single-purpose.
- A domain event is something that happened that another context cares about; name
  it in past tense and name its producer context.
- Quick test for any logic: state → an aggregate; behavior across things → a
  service; something others react to → an event.
